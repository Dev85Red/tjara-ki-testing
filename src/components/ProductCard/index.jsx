...
function ProductsFeature({ detail, isWishlist = false, wishlistItemId, onDeleteWishlist }) {
  ...
  const { showPopup, setopenSigninPopup, setFetchWishlistItems, fetchWishlistItemsCount, cartsItemCount } = usePopup();
  ...
  const renderStockOrRecentBuyer = () => {
    ...
    const notices = [
      detail?.product_type === "auction"
        ...
        : null,
      detail?.sale_price > 0
        ? { text: `${((detail?.price - detail?.sale_price) / detail?.price * 100).toFixed(2)}% Off`, className: "product-meta off" }
        : null,
  ...
  const hasVideo = detail?.video && !detail?.video?.message;
  const thumbnailIsVideo = isVideo(detail?.thumbnail?.media?.url);
  const videoSrc = hasVideo ? getOptimizedVideoUrl(detail?.video) : getOptimizedThumbnailUrl(detail?.thumbnail);
    ...
    detail && (
      <div className="product-card-component feature-products-items-single" >
        <div className={`cardTop ${containerClass}`}>
          ...
          {/* ------------------------------------------------------------------------------------ */}
          {(thumbnailIsVideo) ? (
            <video muted className="product-image" ref={videoRef} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} src={videoSrc} />
          ) : (
            <LazyLoadImage
              effect="blur"
              ...
              src={getOptimizedThumbnailUrl(detail?.thumbnail) || placeholder}
          ...
          {/* ------------------------------------------------------------------------------------ */}
          {(hasVideo) && (
            <video muted className="product-image product-video" onClick={() => navigate(`/product/${detail?.slug}`)} ref={videoRef} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} src={videoSrc} />
          )}
          {/* ------------------------------------------------------------------------------------ */}
          {/*                                      Video Icon                                      */}
          {/* ------------------------------------------------------------------------------------ */}
          {(hasVideo || thumbnailIsVideo) && (
            <div className="product-thumbnail">
              <img src={playVideo} alt="" style={{ position: "absolute", bottom: "31px", right: "3px", width: '25px' }} />
            </div>
          )}
          {/* ------------------------------------------------------------------------------------ */}
          {/*                                    Quick View Tag                                    */}
        ...
        <div className="cardBottom">
          <div className="product-feature-single-store-name-row">
            ...
            <div className="views-count">
              <img src={eyeImage} alt="" className="view-icon" />
          ...
          <Link className="product-feature-single-store-name" to={`/product/${detail?.slug ?? ""}`}>
            ...
            <div className="product-feature-single-price">
              ...
              {detail?.stock == 0 ?
                ...
                detail?.product_type == "variable" || detail?.product_group === "car" ?
                  <p className="viewDetailsBtn" onClick={() => navigate(`/product/${detail?.slug}`)}><svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 512 512"><path fill="#d21642" d="M320 0c-17.7 0-32 14.3-32 32s14.3 32 32 32h82.7L201.4 265.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0L448 109.3V192c0 17.7 14.3 32 32 32s32-14.3 32-32V32c0-17.7-14.3-32-32-32H320zM80 32C35.8 32 0 67.8 0 112V432c0 44.2 35.8 80 80 80H400c44.2 0 80-35.8 80-80V320c0-17.7-14.3-32-32-32s-32 14.3-32 32V432c0 8.8-7.2 16-16 16H80c-8.8 0-16-7.2-16-16V112c0-8.8 7.2-16 16-16H192c17.7 0 32-14.3 32-32s-14.3-32-32-32H80z" /></svg></p>
                  :
