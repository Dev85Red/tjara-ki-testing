...
function ProductPreview() {
  ...
  const [CurrentVariationProduct, setCurrentVariationProduct] = useState(productDetails?.product_type == "auction" || productDetails?.product_type == "simple" ? productDetails : productDetails?.variations?.message ? data?.product : productDetails?.variations?.product_variations[0])
  ...
  if (!productDetails?.video?.message) {
    imageArray.push({
  ...
  if (productDetails?.gallery?.length > 0) {
    productDetails?.gallery?.forEach((image, index) => {
      imageArray.push({
        id: image?.media?.id,
  ...
  if (productDetails?.variations?.product_variations?.length > 0) {
    productDetails?.variations?.product_variations?.forEach((variation, index) => {
      // imageArray.push({
      //   id: variation?.id,
  ...
  return (
    <>
      {isPopupVisible &&
        <section style={{ display: "flex" }} className={`${isPopupVisible && "active"} productPerviewContainer wrapper single-product-details-sec`}>
                  ...
                  <div className="image-preview-container single-product-image-preview-container">
                    {productDetails &&
                      ...
                      (<Swiper
                        ...
                        {imageArray.map((image, index) => (
                          isVideo(image.url) ? (
                            <SwiperSlide style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                              ...
                              <video onCanPlay={handleCanPlay} ref={el => videoRefs.current[index] = el} onClick={(e) => handleVideoPlay(e)} autoPlay key={image.id} style={{ position: videoLoaded ? "static" : "absolute", opacity: videoLoaded ? "1" : "0", height: "100%", objectFit: "cover", background: "#F5F5F5" }} src={image.url} />
                            </SwiperSlide>
                          ) : (
                            <SwiperSlide>
                        ...
                        imageArray.map((image) => (
                          isVideo(image.url) ? (
                            <video key={image.id} style={{ height: "100%", objectFit: "cover" }} src={image.url} />
                          ) : (
