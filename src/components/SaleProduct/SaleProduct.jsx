...
function SaleProduct({ className }) {
  ...
  const fetchOnSaleProducts = async () => {
    const saleProductsSortedIDs = globalSettings?.sale_products_sort_order?.split(',');
    const ApiParams = {
      with: "thumbnail,shop",
      filterByColumns: {
        filterJoin: "AND",
      ...
      orderBy: "ids",
      order: saleProductsSortedIDs,
      ids: saleProductsSortedIDs,
      page: currentPage,
      per_page: 6,
    };
    ...
    <div className={`${className} saleproducts feature-products`}>
      <div className={` feature-products-container`}>
        <div className="feature-products-container-heading-row">
          <p className="feature-product-heading-name">Products On Sale</p>
