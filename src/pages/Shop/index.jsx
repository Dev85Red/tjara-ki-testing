...
const Shop = () => {
  ...
  function handleYearClick(year) {
    // console.log("Selected Model Year:", year);
  }
  ...
  const fetchProducts = async () => {
    ...
    if (data) {
      if (windowsWidth > 550 || currentPage <= 1) {
        ...
        localStorage.setItem('shop-products', JSON.stringify(data.products));
      } else {
        setProducts({ ...products, data: [...products.data, ...data.products.data] });
        localStorage.setItem('shop-products-category', category);
        localStorage.setItem('shop-products', JSON.stringify({ ...products, data: [...products.data, ...data.products.data] }));
      }
  ...
  useEffect(() => {
    if (slug && CurrentCategoryId) {
      fetchSubCategories(CurrentCategoryId == 'products' ? null : CurrentCategoryId);
    }
  ...
  useEffect(() => {
    if (isInitialProductsFetchEffectMount.current || !ProductsFetchTimer) {
  ...
  }, [slug, CurrentCategoryId, searchTerm, orderBy, order, minPriceFilter, maxPriceFilter, countryFilter, stateFilter, cityFilter, categoryFilter, brandFilter, modelFilter, yearFilter, currentPage, ProductsFetchTimer,]);

  /* --------------------------------------------- X -------------------------------------------- */

  useEffect(() => {
    if (localStorage.getItem('shop-products-category') === category) {
      setProducts(JSON.parse(localStorage.getItem('shop-products')));
    }
  }, []);
        ...
        <div className="shop-category-inner-right shop-page-category-inner-right">
          ...
          <div className="shop-category-inner-right-sort-filter-row">
            ...
            <select name="sort" id="sortSelect" onChange={handleSorting}>
              ...
              <option value="featured_desc">Featured {carCategory == true ? "Cars" : "Products"}</option>
          ...
          <div className={`shop-category-inner-right-products-container`}>
            {products == '' ?
              ...
              :
              products?.data?.map((e, i) => (
                e?.product_type == 'auction' ? <AuctionCard key={i} detail={e} /> : <ProductCard cartIcon={cartIcon} key={i} detail={e} />
              ))
            }
          </div>
          {/* ------------------------------------------------------------------------------------ */}
