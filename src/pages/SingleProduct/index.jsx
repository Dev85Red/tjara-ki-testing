  ...
  return (
    <>
            ...
            <div className="product-perview-container">
              <div className="productImagesSlider">
                  ...
                  {imageArrayLength > 1 ? (
                    ...
                    >
                      {imageArray.map((image, index) => (
                        <SwiperSlide key={index} style={{ marginBottom: '10px', border: '1px solid var(--main-red-color)' }}>
                          {isVideo(image.url) ? (
                            <video style={{ width: '100%', height: "100%", objectFit: 'cover' }} src={image.url} />
                          ) : (
                            <img src={image.url} style={{ width: '100%', height: "100%", objectFit: 'cover' }} alt={'Image'} />
                          )}
                        </SwiperSlide>
                      ))}
                        ...
                        product?.variations?.product_variations.length > 0 ? product?.variations?.product_variations?.map((image, index) => (
                          image?.thumbnail?.media?.url ? (
                            <SwiperSlide key={index} style={{ marginBottom: '10px', border: '1px solid var()' }}>
                              <img src={image?.thumbnail?.media?.url} style={{ width: '100%', height: "100%", objectFit: 'cover' }} alt={'Image'} />
                            </SwiperSlide>
                          ) : null
                        )) : null
                      }
                    ...
                    imageArray.map((image, index) => (
                      <div key={index} style={{ marginBottom: '10px', border: '1px solid var()' }}>
                        {isVideo(image.url) ? (
                          <video autoPlay muted={true} controls style={{ height: '80px', objectFit: 'cover' }} src={image.url} />
                        ) : (
                          <img src={image.url} style={{ width: '80px', height: '80px', objectFit: 'cover' }} alt={'Image'} />
                        )}
                      </div>
                    ))
                  ) : ('')}
                ...
                <div className="image-preview-container single-product-image-preview-container">
                  {imageArrayLength > 1 ? (
                      ...
                      {imageArray.map((image, index) => (
                        isVideo(image.url) ? (
                          <SwiperSlide style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }} key={index}>
                            ...
                            <video onCanPlay={handleCanPlay} ref={el => videoRefs.current[index] = el} onClick={(e) => handleVideoPlay(e)} autoPlay style={{ position: videoLoaded ? "static" : "absolute", opacity: videoLoaded ? "1" : "0", height: "100%", objectFit: "cover", background: "#F5F5F5" }} src={image.url} />
                          </SwiperSlide>
                        ) : (
                          <SwiperSlide key={index}>
                            <img onClick={() => { if (window.innerWidth >= 550) { setmainProductImage(fixUrl(image.url)); setMainImagePopup(true); } }} src={fixUrl(image.url)} alt="Image" />
                          </SwiperSlide>
                        )
                      ))}
                      {product?.product_type === 'variable' &&
                    ...
                    imageArray.map((image, index) => (
                      isVideo(image.url) ? (
                        <video key={index} style={{ height: "100%", objectFit: "cover" }} src={image.url} />
                      ) : (
                        <img key={index} onClick={() => { if (window.innerWidth >= 550) { setmainProductImage(fixUrl(image.url)); setMainImagePopup(true); } }} src={fixUrl(image.url)} alt="Image" />
                      )
                    ))
                  ) : ('')}
                </div>
              ...
              <div className="product-perview-details">
                <div className="productPreviewTop">
                  ...
                  <div className={`share ${shareButtons ? "active" : ""}`}>
                    ...
                    <div className="shareButtons">
                      ...
                      {/* <Link to={"https://www.pinterest.com"}><img src={pinterest} alt="" /></Link> */}
                ...
                <div className="product-perview-review-row">
                  <div>{renderStars(product?.reviews?.reviews?.average_rating, RatedStar, EmptyStar)}</div>
      ...
      <div className="fixedContactIcons">
        ...
        <img style={{ cursor: 'pointer' }} onClick={() => setcustomerServicePopup(true)} src={Whatsapp} alt="WhatsApp" />
      </div>
      ...
      {customerServicePopup && (
        <div className="customerServicePopup">
          ...
          <div className="container">
            ...
            <div className="icons">
              {(() => {
                const phone = product?.shop?.shop?.meta?.phone;
                const phoneAreaCode = product?.shop?.shop?.meta?.area_code ?? '';

                return phone && phone !== 'undefined' ? (
                  <Link style={{ cursor: 'pointer' }} to={`tel:${phoneAreaCode ?? ''}${phone}`} className="singleIcon">
                    <div>
                  ...
                  <Link style={{ opacity: '0.5', filter: 'blur(0.5)', pointerEvents: 'none' }} to={''} className="singleIcon">
                    <div>
                      <img src={call} alt="Call" />
                    </div>
                    <p>Call</p>
              ...
              <div style={{ cursor: 'pointer' }} onClick={() => { setcustomerServicePopup(!customerServicePopup); handleChatSubmit(); }} className="singleIcon">
                <div>
                  <img src={chat} alt="" />
                </div>
                <p>Live Chat</p>
              ...
              {(() => {
                const whatsapp = product?.shop?.shop?.meta?.whatsapp;
                const whatsappAreaCode = product?.shop?.shop?.meta?.whatsapp_area_code ?? '';

                return whatsapp && whatsapp !== 'undefined' ? (
                  <Link style={{ cursor: 'pointer' }} target="_blank" to={`https://api.whatsapp.com/send?phone=${whatsappAreaCode ?? ''}${whatsapp}`} className="singleIcon">
                    <div>
